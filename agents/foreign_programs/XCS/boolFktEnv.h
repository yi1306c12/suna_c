...
struct booleanEnv{
  ...
  int carryFunction;
  int combinedParityCOorMPFunction;
...
#define COMBINED_PARITY_MP_OR_CO 0 /* combines partiy with multiplexer or count ones... */
...
#define COMBINED_TYPE 1 /* SPECIFIES WHICH FUNCTION IS UNDERLYING... 0=COUNT_ONES, 1=MULTIPLEXER */
...
int nextTestState(struct attributes *state); /* returns the next test problem */
