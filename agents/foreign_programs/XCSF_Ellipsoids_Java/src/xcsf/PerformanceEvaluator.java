...
public class PerformanceEvaluator {
	 ...
	 *            the current noiseless functionvalue, if available.
		...
		if (index == 0) {
			if (iteration > 0) {
				...
				this.avgPerformance[experiment][iteration
						/ XCSFConstants.averageExploitTrials - 1] = performance;
				if (verbose) {
					System.out.println(performanceToString(iteration,
							performance));
				}
			} else if (verbose) {
	...
	void writeAvgPerformance(String file) {
		...
		// write to file
		try {
			FileWriter fw = new FileWriter(file + EXTENSION);
		...
		try {
			...
			c.writeln("set xlabel 'number of learning steps'");
			c.writeln("set ylabel 'pred. error, macro cl.'");
			...
			c.writeln("set out '" + file + ".eps'");
			c.writeln("plot '" + file + EXTENSION
					+ "' using 1:2:2:($2+$3) title 'pred. error' pt 1,"
					+ " '' using 1:4:4:($4+$5) title 'macro cl.' pt 2");
			c.writeln("save '" + file + ".plt'");
			c.writeln("exit");
	 ...
	 * @see #getHeader() for details of the array contents.
	 */
	private double[] evaluatePerformance(Population population) {
		...
		performance[8] = 0;
		performance[9] = 0;
		for (xcsf.classifier.Classifier cl : population) {
			// 6) predictionerror
