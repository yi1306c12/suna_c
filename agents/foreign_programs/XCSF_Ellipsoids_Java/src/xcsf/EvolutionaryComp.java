...
import xcsf.classifier.ConditionHyperellipsoid;
import xcsf.classifier.RLSPrediction;
...
class EvolutionaryComp {
	...
	final static double LOWER_BOUND = 0.0;
	final static double UPPER_BOUND = 1.0;
	final static double LOWER_ROTATION_BOUND = 0;
	final static double UPPER_ROTATION_BOUND = 2 * Math.PI;

	boolean doCondensation;
	int selectionSize;
	...
	void evolve(Population population, MatchSet matchSet,
		...
		Vector<Classifier> offspring = new Vector<Classifier>(parents.size());
		...
		if (!this.doCondensation) {
			this.crossoverAndMutation(offspring);
		}
	 ...
	 */
	Vector<Classifier> selection(MatchSet matchSet, double fitnessSum) {
		Vector<Classifier> selection = new Vector<Classifier>(
				this.selectionSize);
	 ...
	 * 
	 * @param offspring
	 *            the list of <code>Classifier</code> objects to cross and/or
	 *            mutate.
	 */
	void crossoverAndMutation(Vector<Classifier> offspring) {
	 ...
	 *            the machted state.
	 */
	void insertion(Vector<Classifier> offspring, Vector<Classifier> parents,
	...
	private static void subsumeClassifier(Classifier offspring,
		...
		// 2) check matchSet
		Vector<Classifier> choices = new Vector<Classifier>();
		for (Classifier cl : matchSet) {
	 ...
	 */
	private static boolean uniformCrossover(Classifier cl1, Classifier cl2) {
	 ...
	 * 
	 * @param condition
	 *            the condition to mutate.
	 * @return true, if the transformation matrix needs to be recalculated.
	 */
	private static boolean mutateCondition(ConditionHyperellipsoid condition) {
			...
			for (Classifier cl : matchSet) {
				for (int j = 0; j < cl.getNumerosity(); j++) {
					if (XCSFUtils.Random.uniRand() < XCSFConstants.selectionType) {
						double microFitness = cl.getFitness()
								/ cl.getNumerosity();
						if (ret == null || microFitness > bestFitness) {
							ret = cl;
	 ...
	 * @param arr2
	 */
	private static void flip(int pos, double[] arr1, double[] arr2) {
		double tmp = arr1[pos];
		arr1[pos] = arr2[pos];
		arr2[pos] = tmp;
	}
