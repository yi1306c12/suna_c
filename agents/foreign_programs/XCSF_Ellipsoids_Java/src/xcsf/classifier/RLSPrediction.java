 ...
 */
public class RLSPrediction implements Cloneable {
	 ...
	 */
	public RLSPrediction(int inputLength, double[] initialPrediction) {
	 ...
	 */
	public double[] predict(double[] predInput) {
		for (int p = 0; p < this.predictionLength; p++) {
			// first coefficient is offset
			this.prediction[p] = this.coefficients[p][0]
					* XCSFConstants.predictionOffsetValue;
			// multiply other coefficients with the prediction input
			for (int i = 1; i < this.inputLength; i++) {
				this.prediction[p] += this.coefficients[p][i]
						* predInput[i - 1];
			}
		}
	 ...
	 *            .
	 */
	public void updatePrediction(double[] predInput, double[] functionValue) {
		extendPredictionInput(predInput);
		// tmpArray = gainMatrix * extendedPredInput
		XCSFUtils.multiply(this.gainMatrix, tmpExtendedPredInput, tmpGainArray,
				this.inputLength);
	...
	/**
	 * Extends the given vector with a leading value (see
	...
	private static void extendPredictionInput(double[] predInput) {
		...
		for (int i = 0; i < predInput.length; i++) {
			tmpExtendedPredInput[i + 1] = predInput[i];
		}
	}
}
