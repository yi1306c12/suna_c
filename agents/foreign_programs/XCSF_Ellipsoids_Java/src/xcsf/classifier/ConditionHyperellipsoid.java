 ...
 */
public class ConditionHyperellipsoid implements Cloneable {
	 ...
	 */
	public ConditionHyperellipsoid(double[] conditionInput) {
	...
	public boolean doesMatch(double[] conInput) {
		if (!XCSFUtils.arrayEquals(this.conditionInput, conInput)) {
			this.sqareDistance = calculateRelativeSquaredDistance(conInput);
			this.conditionInput = conInput;
		}
		return this.sqareDistance < 1;
	}
	 ...
	 *         <code>conInput</code>.
	 */
	public double getActivity(double[] conInput) {
		if (!XCSFUtils.arrayEquals(this.conditionInput, conInput)) {
			this.sqareDistance = calculateRelativeSquaredDistance(conInput);
			this.conditionInput = conInput;
		}
		return Math.exp(-this.sqareDistance);
	}
	 ...
	 *         vector.
	 */
	public boolean isMoreGeneral(ConditionHyperellipsoid other) {
		 ...
		 * The old way:
		 * <p>
		 * This method uses a heuristic: at first, the difference vector of the
		 * two centers is calculated. The difference vector is elongenated to
		 * the boundary of the other condition. If this condition still matches
		 * the boundary point of the other condition, this condition is more
		 * general than <code>other</code>.
		 */
		// boolean sameCenter = true;
		// // calculate centerDiff
		// for (int i = 0; i < dimension; i++) {
		// tmpCenterDiff[i] = other.center[i] - this.center[i];
		// if (tmpCenterDiff[i] != 0) {
		// sameCenter = false;
		// }
		// }
		// if (sameCenter) { // compare stretches
		// for (int i = 0; i < dimension; i++) {
		// if (this.stretch[i] < other.stretch[i]) {
		// return false;
		// }
		// }
		// // this.stretch is always greater/equal
		// // FIXMe angles?
		// return MyUtils.arrayEquals(this.angle, other.angle);
		// }
		// // ok, centers are different!
		//
		// // calculate euclidean distance from other.center to other.boundary
		// double dist = other.calculateRelativeDistance(tmpCenterDiff);
		// // stretch centerDiff to boundary of other ellipsoid:
		// // scale with 1.0 + 1/dist
		// dist = 1.0 + 1.0 / dist;
		// for (int i = 0; i < dimension; i++) {
		// tmpCenterDiff[i] *= dist;
		// }
		//
		// // calculate distance from this.center to other.boundary
		// dist = this.calculateRelativeDistance(tmpCenterDiff);
		// return dist < 1.0;
	}
