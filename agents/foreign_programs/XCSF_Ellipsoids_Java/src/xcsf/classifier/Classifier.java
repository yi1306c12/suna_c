...
public class Classifier implements Cloneable {
	 ...
	 * @see xcsf.MatchSet for the covering mechanism.
	 */
	...
	/**
	 * Empty constructor for efficient cloning only.
	 */
	private Classifier() {
		// empty
	}
	 ...
	 * 
	 * @param state
	 *            The current state, which this classifier matches.
	 */
	public void update1(StateDescriptor state) {
		...
		if (experience < 1. / XCSFConstants.beta) {
			...
			predictionError += XCSFConstants.beta
					* (absError - predictionError);
		}
	}
	...
	public void update2(double accuracy, double accuracySum, int numerositySum) {
		...
		this.fitness += XCSFConstants.beta
				* ((accuracy * numerosity) / accuracySum - fitness);
	}
	 ...
	 * 
	 * @param meanFitness
	 *            The mean fitness in the population considered for deletion.
	 ...
	 * 
	 * @return true, if this classifier can subsume others.
	 */
