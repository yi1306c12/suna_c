...
abstract class ClassifierSet implements Iterable<Classifier> {
	 ...
	 * 
	 * @param classifier
	 *            classifier to be appended to this list
	 */
	void add(Classifier classifier) {
		if (this.size == this.elements.length - 1)
			throw new IndexOutOfBoundsException("Succeded maximum capacity: "
	 ...
	 * 
	 * @param index
	 *            Index of the <code>Classifier</code> to return.
	 ...
	 *             if the <code>index</code> is not valid.
	 */
	...
	void remove(int[] indices) {
		...
		for (i = 0; i < indices.length - 1; i++) {
			...
			// shift by 1 + i
			System.arraycopy(this.elements, indices[i] + 1, this.elements,
