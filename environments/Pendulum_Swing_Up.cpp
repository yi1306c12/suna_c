...
void Pendulum_Swing_Up::start(int &number_of_observation_vars, int& number_of_action_vars)
{
...
----------------------------------------------------------------------*/
void Pendulum_Swing_Up::cart_pole(double force, double *x,double *x_dot, double *theta, double *theta_dot) 
{
...
double Pendulum_Swing_Up::step(double* action)
{
	// initial reward
	if(action==NULL)
	{
...
double Pendulum_Swing_Up::restart()
{
	trial++;
	...
	if(RANDOM_START)
	{
		...
		x_dot = 0;
		theta = DEGREES_180 + random->uniform(-0.1, 0.1);
